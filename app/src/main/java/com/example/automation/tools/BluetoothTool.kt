...
import com.example.automation.actions.BluetoothAction
import com.example.automation.engine.ExecutionRequest
import com.example.automation.verification.RetryPolicy
import org.json.JSONObject

class BluetoothTool(private val bluetoothAction: BluetoothAction) : Tool {
    override val name: String = "bluetooth"
    override val supportedActions: Set<String> = setOf("SET_BLUETOOTH", "TOGGLE_BLUETOOTH", "OPEN_BLUETOOTH")
    ...
    override suspend fun execute(context: Context, request: ExecutionRequest): ToolResult {
        return try {
            ...
            bluetoothAction.execute(context, jsonPayload)
            ToolResult(
                success = true,
                toolName = name,
            ...
            ToolResult(
                ...
                message = e.message ?: "Bluetooth operation failed"
            )
        }
    }
}
